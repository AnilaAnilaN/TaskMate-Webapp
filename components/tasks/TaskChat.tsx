// components/tasks/TaskChat.tsx
'use client';

import { useState, useEffect, useRef } from 'react';
import { Send, Trash2, Loader2, Sparkles, MessageCircleMore, X } from 'lucide-react';
...
const formatMessage = (text: string) => {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return (
        <code key={i} className="px-1.5 py-0.5 bg-gray-200 rounded text-sm font-mono text-gray-800">
          {part.slice(1, -1)}
        </code>
      );
    }
    ...
    return lines.map((line, j) => {
      ...
      if (line.trim().startsWith('###')) {
        return (
          <h4 key={`${i}-${j}`} className="font-semibold text-base mt-3 mb-1 text-gray-900">
      ...
      if (line.trim().startsWith('##')) {
        return (
          <h3 key={`${i}-${j}`} className="font-bold text-lg mt-4 mb-2 text-gray-900">
...
export default function TaskChat({ taskId, isOpen, onClose }: TaskChatProps) {
  ...
  const sendMessage = async () => {
    ...
    const tempUserMsg: TaskChatMessage = {
      id: `temp-${Date.now()}`,
      taskId,
      userId: 'current-user',
      role: 'user',
    ...
    try {
      ...
      const errorMsg: TaskChatMessage = {
        id: `error-${Date.now()}`,
        taskId,
        userId: 'system',
        role: 'assistant',
  ...
  const clearHistory = async () => {
    if (!confirm('Clear all chat history for this task?')) return;

    try {
      const response = await fetch(`/api/tasks/${taskId}/chat`, { method: 'DELETE' });
      const data = await response.json();
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
      <div className="w-full h-full md:h-150 md:max-w-2xl bg-white md:rounded-2xl shadow-xl flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-gray-900">Task Assistant</h2>
          ...
          {initialLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-gray-500 max-w-md mb-6 text-sm">
                I have context about this task and can help you with specific suggestions, breakdowns, and advice.
              </p>
              <div className="space-y-2 w-full max-w-md">
                <button
                  onClick={() => setInput('How should I break down this task?')}
                  className="block w-full px-4 py-2 text-sm text-left bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
                >
                  💡 How should I break this down?
                </button>
                <button
                  onClick={() => setInput('What are the key steps to complete this?')}
                  className="block w-full px-4 py-2 text-sm text-left bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
                >
                  📋 What are the key steps?
                </button>
                <button
                  onClick={() => setInput('Any tips for completing this efficiently?')}
                  className="block w-full px-4 py-2 text-sm text-left bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
                >
                  🚀 Tips for efficiency?
                </button>
            ...
            messages.map((msg) => (
              ...
              >
                {msg.role === 'assistant' && (
                  <div className="w-8 h-8 bg-linear-to-br from-yellow-400 to-orange-500 rounded-lg flex items-center justify-center shrink-0 mt-1">
        ...
        {/* Input */}
        <div className="border-t border-gray-200 p-4 md:p-6 shrink-0 bg-gray-50">
